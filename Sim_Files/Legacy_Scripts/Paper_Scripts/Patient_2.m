function Patient_2(conmul, input_state, EGM_name, EGM_features_name, ...
    ATP_CL, ATP_coupling, ATP_pls, ATP_dec, ATP_Min_Cycle)

%Assign default values for input arguments if not provided
    if nargin < 9, ATP_Min_Cycle = 220.00; end
    if nargin < 8, ATP_dec = 5.00; end
    if nargin < 7, ATP_pls = 12.00; end
    if nargin < 6, ATP_coupling = 0.88; end
    if nargin < 5, ATP_CL = 0.88; end
    if nargin < 4, EGM_features_name = 'EGM_features_reentrant_LV'; end
    if nargin < 3, EGM_name = 'EGM_reentrant_LV'; end
    if nargin < 2, input_state = '2_LV_top_3550.roe'; end
    if nargin < 1, conmul = 0.25; end
...
FolderName = sprintf('%s_Patient_2', char(todayDate));
...
% Episode parameters ofr patient 2:
Simscript = 'Basic_BiV_Infarcted_VT_Episode.py'; %Choose the simulation type: NSR, Focal or Infarcted Reentrant
mesh = 'ventricles_coarser_LV_infarct'; %Choose the type of mesh, normal structure or the left/right ventricle infarction
monitor_duration = 28800; %How long the ICD monitors the simulation (in seconds)
...
% Display the output file name and the pid
...
[ICD_diagnosis, EGM, EGM_features, ICD_sense_state, ICD_sense_param] = monitor_initial_reentrant(monitor_duration, outputFile, EGM_name, EGM_features_name, NSR_temp, tend);
...
while(true)
    ...
    elseif VF && history(3) < 2 %QC therapy required - Follow therapy pathway for VF:
        ...
        disp('Running VF ATP - Quick Convert')
        [~, Redetect_param] = ATP_Burst_Therapy_reentrant(ATP_param, mesh, conmul, nprocs, pythonExe, full_sim_time, bcl, check);
        ...
        [outputFile] = runVentricularSimulation(Simscript, mesh, conmul, input_state, 'tenTusscherPanfilov', tend, pls, bcl, strength, duration, start, output_res, check, nprocs, pythonExe);
        ...
        % simulation
        [ICD_diagnosis, ~, ~, ~, ~] = monitor_redetect_reentrant(monitor_duration, outputFile, EGM_name_post_therapy, EGM_features_name_post_therapy, prev_therapySigs, Input_state_time, NSR_temp);
    ...
    elseif VT && history(2) < 2 % VT ATP_1 therapy required
        ...
        [~, Redetect_param] = ATP_Burst_Therapy_reentrant(ATP_param, mesh, conmul, nprocs, pythonExe, full_sim_time, bcl, check, NSR_temp);
    ...
    elseif VT >= 1 && history(2) >= 2 % VT ATP_2 therapy required
        ...
        [~, Redetect_param] = ATP_Ramp_Therapy_reentrant(ATP_param, mesh, conmul, nprocs, pythonExe, full_sim_time, bcl, check);
    ...
    atpfiles = dir(fullfile(currentDir, ['EGM_ATP', '*']));

    % Get a list of matching files
    allfiles = [tracefiles; featurefiles, atpfiles];
    ...
    trcfiles = dir(fullfile(currentDir, 'Sim_Files', 'Episode_Sim_Scripts', 'Reentrant_VT', ['*','.trc']));
    chkptfiles = dir(fullfile(currentDir, 'Sim_Files', 'Episode_Sim_Scripts', 'Reentrant_VT', ['*', '.roe']));
    petscfolders = dir(fullfile(currentDir, ['*', 'petsc', '*']));

    for i = 1:length(trcfiles)
        filePath = fullfile(currentDir, 'Sim_Files', 'Episode_Sim_Scripts', 'Reentrant_VT', trcfiles(i).name);
        delete(filePath);
    end

    for i = 1:length(chkptfiles)
        filePath = fullfile(currentDir, 'Sim_Files', 'Episode_Sim_Scripts', 'Reentrant_VT', chkptfiles(i).name);
        delete(filePath);
    end
