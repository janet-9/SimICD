...
def parser():
   ...
   #Length of full simulation run-time - remember the time stamp of the input state! 
    ...
    group.add_argument('--ATP_dec',
                       ...
                       help = 'Decrement of the ATP cycle length during a burst (default is %(default)s)') # Ramp decrement 
...
# Set the base number of stimuli (5 NSR stimuli)
    cmd += ['-num_stim', 5 + int(args.ATP_pls)]  # Add the number of ATP pulses to the total number of stimuli
    #print(5 + int(args.ATP_pls))
# Add NSR stimuli (as before)
    ...
    for i in range(int(args.ATP_pls)):
        ...
        cmd += [
                ...
                f'-stimulus[{5 + i}].vtx_file', args.ATP_stimsite,
                f'-stimulus[{5 + i}].stimtype', 0,
                f'-stimulus[{5 + i}].strength', args.ATP_strength,
                f'-stimulus[{5 + i}].duration', args.ATP_duration,
                f'-stimulus[{5 + i}].start', atp_start_times[i],
                f'-stimulus[{5 + i}].npls', 1,
            ]
