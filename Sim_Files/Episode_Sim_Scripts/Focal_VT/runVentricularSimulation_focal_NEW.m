function [outputFile] = runVentricularSimulation_focal_NEW(Simscript, nprocs, pythonExe, tend, args)
...
% Output file name is saved to match the one generated by the carputils script %
outputFile = sprintf('%s_%s_Focal_VT_%s_%.2f_episodes_%.2f_focal_bcl_%.2f_focal_pls_%.2f_focal_strength_%.2f_focal_duration', todayDateStr, args.input_state, args.focal_site, args.episodes, args.focal_bcl, args.focal_pls, args.focal_strength, args.focal_duration);
...
fileMap = containers.Map(...
    { 'input_state', 'NSR_vtx', 'focal_site', 'ATP_stimsite', 'EGM_template'}, ...
    { 'input_states', 'stim_sites', 'stim_sites', 'stim_sites' 'NSR_temps'} ...
);
...
cmd = sprintf('%s %s --np %d --mesh "%s" --myocardium %.2f --scar_flag %d --scar_region %.2f --isthmus_region %.2f --conmul %.2f --input_state "%s" --model "%s" --tend %.2f --bcl %.2f --strength %.2f --duration %.2f --start %.2f --NSR_vtx "%s" --electrodes "%s" --output_res %.2f --check %.2f --focal_site "%s" --focal_start %.2f --episodes %.2f --episode_interval %.2f --focal_pls %.2f --focal_bcl %.2f --focal_strength %.2f --focal_duration %.2f & echo $!', ...
    pythonExe, Simscript, nprocs, args.mesh, args.myocardium, args.scar_flag, args.scar_region, args.isthmus_region, args.conmul, args.input_state, args.model, tend, args.bcl, args.strength, args.duration, args.start, args.NSR_vtx, args.electrodes, args.output_res, args.check, args.focal_site, args.focal_start, args.episodes, args.episode_interval, args.focal_pls, args.focal_bcl, args.focal_strength, args.focal_duration);
