function B_Reentrant_VT_Patient(varargin)
...
%% Define simulation parameters: set the default values 
defaults = {
    'EGM_reentrant_VT', ... % Name for the EGM
    'EGM_features_reentrant_VT', ... % name for the EGM features
...
[ EGM_name, EGM_features_name, ATP_CL, ...
    ...
    duration, start, NSR_temp, NSR_vtx, electrodes, output_res, check,...
    ATP_strength, ATP_duration, ATP_stimsite, ...
    max_therapy_calls] = defaults{:};


% Print out the default parameters to the user in a concise format
disp('SimICD launched! Reentrant Episode with Nominal Progression');
...
%% Simulation and Diagnosis Stage:
...
% Call the function to run the simulation in the episode subfolder
% (background process)
[outputFile] = runVentricularSimulation_reentrant(Simscript, mesh, myocardium, scar_flag, scar_region, isthmus_region, conmul, input_state, model, tend, bcl, strength, duration, start, NSR_vtx, electrodes, output_res, check, nprocs, pythonExe);
...
% required OR the 30 second episode ends without the need for therapy,
% and returns the necessary therapy parameters.

[ICD_diagnosis, ~, ~, ~, ~] = monitor_initial_reentrant(monitor_duration, outputFile, EGM_name, EGM_features_name, NSR_temp, tend);
...
while(true)
    ...
    elseif VF && history(3) < 1 %QC therapy required - Follow therapy pathway for VF:
        ...
        disp('Running QC ATP...')
        [~, Redetect_param] = ATP_Burst_Therapy_reentrant(ATP_param, mesh, myocardium, scar_flag, scar_region, isthmus_region, conmul, nprocs, pythonExe, full_sim_time, bcl, check, model, strength, duration, start, NSR_vtx, electrodes, output_res);
    ...
    elseif VT >= 1 && history(2) >= 2 % VT ATP_2 therapy required
        ...
        [~, Redetect_param] = ATP_Ramp_Therapy_reentrant(ATP_param, mesh, myocardium, scar_flag, scar_region, isthmus_region, conmul,model, strength, duration, start, NSR_vtx, electrodes, output_res, nprocs, pythonExe, full_sim_time, bcl, check);
    ...
    simfiles = dir(fullfile(currentDir, 'Sim_Files', 'Episode_Sim_Scripts', 'Reentrant_VT', [todayDate,'*']));
    ...
    %2: Clear the trc, checkpoints, state files and petsc folders from the simulation script
