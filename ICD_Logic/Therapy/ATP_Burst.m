function [outputFile, ICD_diagnosis, roeFile, ATP_param] = ATP_Burst(outputFile, ICD_diagnosis, Input_state_time, ATP_CL, ATP_coupling, mesh, conmul, nprocs, pythonExe, Simscript, full_sim_time, bcl, strength, duration, start, output_res, check, monitor_duration)
    ...
    Input_state_time = str2double(input_state_time{end}); %This should be the time of the input_state
    ...
    [ICD_diagnosis, ~, ~, ~, ~] = monitor_redetect(monitor_duration, outputFile, EGM_name_post_therapy, EGM_features_name_post_therapy, prev_therapySigs, Input_state_time);
