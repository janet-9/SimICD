function Patient_3(conmul, input_state, EGM_name, EGM_features_name, ...
...
%Assign default values for input arguments if not provided
    ...
    if nargin < 7, ATP_pls = 10.00; end
    if nargin < 6, ATP_coupling = 0.88; end
    if nargin < 5, ATP_CL = 0.88; end
    if nargin < 4, EGM_features_name = 'EGM_features_reentrant_RV'; end
    if nargin < 3, EGM_name = 'EGM_reentrant_RV'; end
    if nargin < 2, input_state = '3_RV_bottom_3550.roe'; end
    if nargin < 1, conmul = 0.18; end
...
FolderName = sprintf('%s_Patient_3', char(todayDate));
...
mesh = 'ventricles_coarser_RV_infarct'; %Choose the type of mesh, normal structure or the left/right ventricle infarction
...
while(true)
    ...
    elseif VT >= 1 && history(2) >= 2 % VT ATP_2 therapy required
        ...
        [~, Redetect_param] = ATP_Ramp_Therapy_reentrant(ATP_param, mesh, conmul, nprocs, pythonExe, full_sim_time, bcl, check, NSR_temp);
